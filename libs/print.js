...
let urlRegex = (await import('url-regex-safe')).default({ strict: false });
import PhoneNumber from 'awesome-phonenumber';
import chalk from 'chalk';
import { watchFile } from 'fs';

let terminalImage = global.opts?.img ? (await import('terminal-image')).default : '';

export default async function (m, conn = { user: {} }) {
  const senderJid = (await conn.signalRepository.lidMapping.getPNForLID(m.sender))?.split('@')[0].split(':')[0] + "@s.whatsapp.net"
  let _name = m.name
  let sender = PhoneNumber('+' + senderJid.replace('@s.whatsapp.net', '')).getNumber('international') + (_name ? ' ~' + _name : '');
